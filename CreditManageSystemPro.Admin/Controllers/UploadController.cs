...
namespace CreditManageSystemPro.Admin.Controllers
{
    ...
    {
        public ActionResult ProcessRequest(string action){
            JsonResult json=new JsonResult();
            HttpContext contexts = HttpContext.Current;
            switch (action)
            {
                case "EditorFile": //编辑器文件
                    EditorFile(contexts,json);
                    break;
                case "ManagerFile": //管理文件
                    ManagerFile(contexts,json);
                    break;
                default: //普通上传
                    UpLoadFile(contexts,json);
                    break;
            }
            return json;
        }
        ...
        #region 编辑器上传处理===================================
        private void EditorFile(HttpContext context, JsonResult json)
        {
            ...
            Dictionary<string, object> dic = JSON.DataRowFromJSON(remsg);
            ...
            context.Response.Write(JSON.ObjectToJSON(hash));
        ...
        #region 上传文件处理===================================
        private void UpLoadFile(HttpContext context, JsonResult json)
        {
            siteconfig siteConfig = new siteconfigs().loadConfig();
        ...
        #region 浏览文件处理=====================================
        private void ManagerFile(HttpContext context, JsonResult json)
        {
            siteconfig siteConfig = new siteconfigs().loadConfig();
            ...
            context.Response.Write(JSON.ObjectToJSON(result));
            context.Response.End();
        }
