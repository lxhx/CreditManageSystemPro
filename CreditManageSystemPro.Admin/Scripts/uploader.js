...
$(function () {
    ...
    $.upLoadDefaults.property = {
        ...
        filesize: "2048", //文件大小
        btntext: "浏览...", //上传按钮的文字
        btnPretext: "预览", //上传按钮的文字
        prepic: true, //是否展示预览图片 
        hideinput: false, //是否隐藏上传文件地址控件 
    ...
    $.fn.InitUploader = function (b) {
        var fun = function (parentObj) {
            ...
            var btnObj = $('<div class="upload-btn">' + p.btntext + '</div>');
            btnObj.appendTo(parentObj);
            var prevId = parentObj.prev().attr("id");
            var _preObj;


            if (p.hideinput) {
                parentObj.parent().find(".upload-path").hide();//隐藏上传路径控件 
            } else {
                parentObj.parent().find(".upload-path").blur(function () {
                    showPrePic(prevId, _preObj);
                });
            }


            //删除图片 
            var btnDelObj = $('<img src="/images/human91.png" style="margin-bottom:7px;margin-left:10px;margin-right:0px;cursor:pointer;" alt="删除图片" />');
            btnDelObj.appendTo(parentObj.parent().find(".upload-img"));
            btnDelObj.click(function () {
                delPrePic(btnDelObj, prevId);
            });

            var btnPreObj = $('<img src="/images/human90.png" style="margin-bottom:7px;margin-left:10px;margin-right:10px;cursor:pointer;" alt="预览图片" />');
            btnPreObj.appendTo(parentObj.parent().find(".upload-img"));
            btnPreObj.click(function () {
                top.dialog({
                    id: 'prePicDialogId',
                    title: "",
                    padding: "10px",
                    quickClose: true,// 点击空白处快速关闭
                    content: "<a href=\"" + $("#" + prevId).val() + "\" target=\"_blank\"><img width=\"160\" src=\"" + $("#" + prevId).val() + "\" /></a>",
                }).show(this);
                return false;
            });
            //alert(parentObj.parent().html());




            //生成预览
            if (!p.multiple && p.prepic) {
                _preObj = addPrePic(prevId, parentObj);
            }

            //初始化属性
            //p.sendurl = (p.sendurl!="" && p.sendurl.indexOf('?')==-1) ? p.sendurl : "../../tools/upload_ajax.ashx"; //发送地址
...
//显示图片预览，附加到上传地址下面 
function addPrePic(_prevId, _parentObj) {
    var _preObj = $('<div class="upload-pre" id="upload-' + _prevId + '" style="margin-top:8px;"><a id="a_' + _prevId + '" href="" target="_blank"><img src="" id="img_' + _prevId + '" alt="点击查看源图" width="160" /></a></div>');
    if ($("#" + _prevId).val() != "") {
        _preObj.find("#a_" + _prevId).attr("href", $("#" + _prevId).val());
        _preObj.find("#img_" + _prevId).attr("src", $("#" + _prevId).val());
        _preObj.appendTo(_parentObj.parent());
        _ispreload = 1;
    }
    return _preObj;
}

function showPrePic(_prevId, _preObj) {
    _preObj.find("#a_" + _prevId).attr("href", $("#" + _prevId).val());
    _preObj.find("#img_" + _prevId).attr("src", $("#" + _prevId).val());
}

function delPrePic(obj, _prevId) {
    top.dialog({
        title: '提示',
        content: "确定要清空图片么，图片本身不会被删除",
        okValue: '确定',
        ok: function () {
            $("#" + _prevId).val("");
            $("#upload-" + _prevId + "").hide();
