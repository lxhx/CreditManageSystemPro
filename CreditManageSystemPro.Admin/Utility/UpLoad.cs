...
namespace CreditManageSystemPro.Admin.Utility
{
    public class UpLoad
    {
        ...
        /// 文件上传方法
        /// </summary>
        /// <param name="imgbase64">图片base64字符串</param>
        /// <param name="isThumbnail">是否生成缩略图</param>
        /// <param name="isWater">是否打水印</param>
        /// <returns>上传后文件信息</returns>
        public string fileSaveAs(string imgbase64, bool isThumbnail, bool isWater)
        {
            try
            {
                string fileExt = "jpg";
                ...
                byte[] postPicBytes = Utils.Base64_Decode(imgbase64);

                MemoryStream ms = new MemoryStream(postPicBytes);
                Image img = Image.FromStream(ms);

                imgpath = newFilePath;
                string filename = fullUpLoadPath + newFileName;
                img.Save(filename);
                ...
                //处理完毕，返回JOSN格式的文件信息
                return "{\"status\": 1, \"msg\": \"上传文件成功！\", \"path\": \"" + newFilePath + "\", \"thumb\": \""
                    + newThumbnailPath + "\"}";
            }
            catch
            {
